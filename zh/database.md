...
### 3. 替代方案：基于注解的映射 *(v1.7.34 中的新特性)*

从 v1.7.34 开始，XML 映射文件是**可选的**。你可以直接在 POJO 上使用 Java 注解声明完整的表到类映射：

| 注解 | 目标 | 用途 |
|---|---|---|
| `@Table` | 类 | 将该类映射到命名的数据库表 |
| `@Id` | 字段 | 标记主键字段 |
| `@Column` | 字段 | 使用名称、类型和长度将字段映射到特定列 |
...
public class User extends AbstractData {
    ...
    // Setter 仍必须调用 setFieldAs* 以在 ORM 中注册值
    public void setEmail(String email) {
        this.email = this.setFieldAsString("email", email);
    }
    public String getEmail() { return this.email; }
...
不需要 `User.map.xml` 文件 —— 框架会在运行时读取这些注解。

> **注意：** Setter 中的 `setFieldAs*` 调用仍然是必需的。注解仅取代 XML 文件；ORM 内部的字段注册表仍然依赖这些调用来获悉应将哪些字段包含在 SQL 语句中。

#### 启用表自动创建

在 `@Table` 中添加 `autoCreate = true`，框架将在首次使用该实体时执行 `CREATE TABLE IF NOT EXISTS` 语句。这在开发和测试期间非常有用：
...
> **警告：** 自动创建在默认情况下是关闭的。它仅适用于开发和测试环境。不要在生产环境中使用它，生产环境应通过数据迁移来管理架构更改。

#### 在 XML 和注解映射之间进行选择

| | XML 映射 | 注解映射 |
|---|---|---|
| 可用版本 | v1.0 及以上 | v1.7.34 及以上 |
| 配置位置 | 单独的 `.map.xml` 文件 | POJO 类内联 |
| 自动建表 | 不支持 | 支持，通过 `autoCreate = true` |
| 运行时开销 | 首次加载时的文件 I/O | 首次加载时的反射 |
| 推荐用于 | 传统 / 复杂映射 | 新代码，快速原型设计 |

这两种方法都受全面支持，并且可以在同一个项目中并存。
